...
function save() {
  try {
    localStorage.setItem(storeKey, JSON.stringify({
      unit: state.unit, rent: state.rent, vacantDays: state.vacantDays,
      cap: state.cap, rooms: state.rooms, issues: state.issues, notes: state.notes
    }));
  } catch {}
}
...
function restore() {
  try {
    ...
    if (Array.isArray(data.issues)) {
      state.issues = data.issues.filter((i) => i && ISSUE_TYPES.some((t) => t.id === i.type)).map((i, idx) => ({
...
function addIssue() {
  ...
  if (!ISSUE_TYPES.some((t) => t.id === type)) { toast("Pick an issue type"); return; }
...
function onFiles(files) {
  ...
  if (!incoming.length) { toast("Photo limit is " + MAX_PHOTOS); return; }
  incoming.forEach((file) => {
    if (!file.type.startsWith("image/")) return;
    if (file.size > 6 * 1024 * 1024) { toast("Photo too large (6MB max)"); return; }
...
function render() {
  ...
  $("#rooms").innerHTML = ROOMS.map((r) => "<button type='button' class='chip " + (state.rooms.includes(r) ? "on" : "") + "' data-room='" + escapeHtml(r) + "'>" + escapeHtml(r) + "</button>").join("");
  $("#photos").innerHTML = state.photos.map((p, i) => "<div class='photo'><img src='" + p.src + "' alt=''><button type='button' class='x' data-photo='" + i + "' aria-label='Remove photo'>x</button></div>").join("") + "<label class='add-photo'>Add photo<input type='file' accept='image/*' multiple capture='environment' id='file-in'></label>";
  ...
  $("#issues").innerHTML = state.issues.length ? state.issues.map((i) => {
  ...
  }).join("") : "<p class='legal'>No issues yet. Add what you see while you walk.</p>";
  $("#cart").innerHTML = cart.items.map((s) => "<div class='cart-item'><div>" + escapeHtml(s.name) + (s.qty > 1 ? " x" + s.qty : "") + "<span class='badge " + (s.affiliate ? "aff" : "no") + "'>" + (s.affiliate ? "May earn" : "No affiliate") + "</span><div class='merchant'>" + escapeHtml(s.merchant) + "</div></div><div class='price'>" + money(s.price * s.qty) + "</div></div>").join("") || "<p class='legal'>Cart fills from tagged issues and the selected cap.</p>";
  ...
  $("#stage").innerHTML = heroPhoto ? "<img src='" + heroPhoto.src + "' alt=''><div class='mark'>Virtually staged - not current condition</div>" : "<div class='empty'>Add a photo to preview the listing still.</div>";
  $("#file-in") && $("#file-in").addEventListener("change", (e) => onFiles(e.target.files));
}
...
function bind() {
  ...
  $("#caps").addEventListener("click", (e) => { const btn = e.target.closest(".cap"); if (!btn) return; state.cap = Number(btn.dataset.cap); render(); });
  $("#rooms").addEventListener("click", (e) => { const btn = e.target.closest("[data-room]"); if (!btn) return; const r = btn.dataset.room; state.rooms = state.rooms.includes(r) ? state.rooms.filter((x) => x !== r) : state.rooms.concat([r]); render(); });
  $("#photos").addEventListener("click", (e) => { const x = e.target.closest("[data-photo]"); if (!x) return; state.photos.splice(Number(x.dataset.photo), 1); render(); });
  $("#issues").addEventListener("click", (e) => { const b = e.target.closest("[data-del]"); if (!b) return; state.issues = state.issues.filter((i) => i.id !== Number(b.dataset.del)); render(); });
  $("#add-issue").addEventListener("click", addIssue);
  $("#issue-note").addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); addIssue(); } });
  $("#sample").addEventListener("click", loadSample);
  $("#print").addEventListener("click", () => window.print());
  $("#reset").addEventListener("click", () => { Object.assign(state, Object.assign({}, DEFAULTS, { rooms: DEFAULTS.rooms.slice(), photos: [], issues: [] })); render(); toast("Turn cleared"); });
}

document.addEventListener("DOMContentLoaded", () => { restore(); bind(); render(); });
